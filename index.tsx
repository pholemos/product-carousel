...
const FILTER_RULES: Record<SupportedRegion, string[]> = {
  'UK': ['UK', 'aliexpress'],
  'USA': ['USA', 'aliexpress'],
...
function getUserRegion(): SupportedRegion {
  try {
...
function filterProducts(products: Product[], region: SupportedRegion): Product[] {
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const initData = async () => {
      try {
        setLoading(true);
        const [rawProducts, userRegion] = await Promise.all([fetchProducts(), Promise.resolve(getUserRegion())]);
  ...
  const filteredProducts = useMemo(() => filterProducts(products, region), [products, region]);
